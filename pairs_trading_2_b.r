...
portfolio_list <- list()
for (pair in pair_names) {
  pairs_signal_col <- paste0(pair, "_Signal")
  ...
  for (i in 1:nrow(sp500_data_adj_strat)) {
    current_signal <- sp500_data_adj_strat[[pairs_signal_col]][i]
    ...
    if (is.null(current_position) || is.na(current_position) || current_position == "") {
      ...
      if ((sp500_data_adj_strat[[spread_zscore]][i] <= 0.2) || (i == nrow(sp500_data_adj_strat))) {
        ...
        if (i < nrow(sp500_data_adj_strat)) {
          ...
          print(ret_value)
          print('First stock')
          print(sp500_data_adj_strat[[first_cumret_col]][i])
          print(portfolio$enter_cumret_first_stock)
          print('Second stock')
          print(portfolio$enter_cumret_second_stock)
          print(sp500_data_adj_strat[[second_cumret_col]][i])
          print("Long return")
          print(long_pos_return)
          print("Short return")
          print(short_pos_return)
         }
        
          
      } else if (sp500_data_adj_strat[[spread_zscore]][i] > 0.2) {
        next
      } else {
...
windows()
plot(sp500_data_adj_strat$Date, sp500_data_adj_strat$MSFT_cumret, type = "l", col = "blue", ylim = c(-0.5, 1.5), 
...
# 2. Create a full date sequence from your strategy period
full_dates <- as.Date(sp500_data_adj_strat$Date)

# 3. Merge all returns series by Date (outer join)
all_returns_merged <- do.call(merge, c(returns_xts_list, all = TRUE))

# 4. Reindex to include all dates, filling missing dates with NA
all_returns_merged <- merge(
  xts(matrix(NA, nrow = length(full_dates), ncol = ncol(all_returns_merged)), order.by = full_dates),
  all_returns_merged,
  fill = NA
)

# 5. Replace NAs with 0 (no trade on that date for that portfolio)
all_returns_merged[is.na(all_returns_merged)] <- 0

all_returns_merged

# 6. Sum returns across portfolios for each date
portfolio_total_returns <- xts(rowSums(all_returns_merged), order.by = index(all_returns_merged))

portfolio_total_returns

# 7. Calculate Sharpe Ratio for the summed portfolio
